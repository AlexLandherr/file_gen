...
namespace prefix {
    std::string to_binary_prefix(uint64_t bytes) {
        ...
        if (bytes > std::numeric_limits<uint64_t>::max() || bytes < std::numeric_limits<uint64_t>::min()) {
            throw std::invalid_argument("Argument 'bytes' is out of range for 'uint64_t' type.");
        }
        ...
        if (bytes < KiB) {
            return round::round_to_str(bytes, 0) + " B";
        } else if (KiB <= bytes && bytes < MiB) {
            return round::round_to_str(bytes / (KiB / 1.0)) + " KiB";
        } else if (MiB <= bytes && bytes < GiB) {
            return round::round_to_str(bytes / (MiB / 1.0)) + " MiB";
        } else if (GiB <= bytes && bytes < TiB) {
            return round::round_to_str(bytes / (GiB / 1.0)) + " GiB";
        } else if (TiB <= bytes && bytes < PiB) {
            return round::round_to_str(bytes / (TiB / 1.0)) + " TiB";
        } else if (PiB <= bytes && bytes < EiB) {
            return round::round_to_str(bytes / (PiB / 1.0)) + " PiB";
        } else {
            return round::round_to_str(bytes / (EiB / 1.0)) + " EiB";
        }
    }
    ...
    std::string to_decimal_prefix(uint64_t bytes) {
        ...
        if (bytes < kB) {
            ...
            return round::round_to_str(bytes / (kB / 1.0)) + " kB";
        } else if (MB <= bytes && bytes < GB) {
            return round::round_to_str(bytes / (MB / 1.0)) + " MB";
        } else if (GB <= bytes && bytes < TB) {
            return round::round_to_str(bytes / (GB / 1.0)) + " GB";
        } else if (TB <= bytes && bytes < PB) {
            return round::round_to_str(bytes / (TB / 1.0)) + " TB";
        } else if (PB <= bytes && bytes < EB) {
            return round::round_to_str(bytes / (PB / 1.0)) + " PB";
        } else {
            return round::round_to_str(bytes / (EB / 1.0)) + " EB";
        }
    }
}